      ...
      <div className="grid grid-cols-2 gap-6 mt-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/applications")} className="text-xs text-orange-500 font-medium hover:underline flex items-center gap-0.5">
          ...
          {interviews.length === 0 ? (
            ...
            <div className="space-y-2">
              {interviews.map((iv) => (
                <div key={iv._id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50/80 transition-colors cursor-pointer" onClick={() => navigate("/applications")}>
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-purple-100 flex items-center justify-center shrink-0">
                      <Calendar size={16} className="text-purple-600" />
                    </div>
                    <div>
                      <p className="font-medium text-sm text-gray-800">{iv.job?.title}</p>
                      <p className="text-xs text-gray-400 capitalize">{iv.mode || "TBD"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-gray-500 shrink-0">
                    <Clock size={13} />
                    {new Date(iv.scheduledAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => navigate("/jobs")} className="text-xs text-orange-500 font-medium hover:underline flex items-center gap-0.5">
...
300 mt-0.5">Browse jobs and save your favorites</p><p className="text-xs text-gray-300 mt-0.5">Save jobs to apply later</p>
</div>
) : (
<div className="space-y-2">
  {savedJobsList.map((job) => (
    <div key={job._id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50/80 transition-colors cursor-pointer" onClick={() => navigate(`/jobs/${job._id}`)}>
      <div className="flex items-center gap-3">
...
{/* Profile Strength — now with real data + checklist */}
